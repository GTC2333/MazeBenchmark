...
| **O**（重合度） | 覆盖比例 | 模型路径与最短路径的集合重合度（交/并），范围 [0,1] |
| **R**（鲁棒性） | 抗扰动能力 | 在路径中施加微小扰动后，是否仍可通过贪心修复抵达终点？✅=1，否则 0 |
| **A**（防作弊） | 合规性   | 通过输出沙盒检查（仅坐标字符、无注入/解释性文本）✅=1，否则 0 |

📈 综合得分 = `w_S·S + w_Q·Q + w_O·O + w_R·R + w_A·A`，最终以 **百分制** 展示。
> 目前默认权重（随尺寸变化但总体趋势一致）在 `Metrics` 中实现：小尺寸与大尺寸均为示例值 `S:0.5, Q:0.2, O:0.2, R:0.08, A:0.02`。

#### 微小扰动（Robustness 解释）
- 在评测阶段，我们对模型生成的路径注入“微小扰动”（如在中间若干步选择一个相邻可行格进行轻微偏移），随后使用一个**贪心纠偏器**尝试继续向终点推进。
- 若在限定步数内仍能回到正确轨迹并到达终点，则认为该解在局部扰动下具备可恢复性，记为 `R=1`，否则 `R=0`。
- 该设计旨在评估路径的“局部可修复性”，与输入提示的扰动解耦，避免把“提示解析鲁棒性”与“路径规划鲁棒性”混为一谈。
